...
public class PrivacyControllerTest {
    ...
    public void recordConsent_Success() throws Exception {
        ...
        when(privacyService.recordConsent(
                eq("user123"),
                eq("DATA_PROCESSING"),
                eq(true),
                any(),
    ...
    public void getUserConsents_Success() throws Exception {
        ...
        when(privacyService.getUserConsents("user123"))
                .thenReturn(Collections.singletonList(consent));

        mockMvc.perform(get("/api/v1/privacy/consent/user123"))
                .andExpect(status().isOk())
    ...
    public void deleteUserData_Success() throws Exception {
        mockMvc.perform(delete("/api/v1/privacy/user/user123"))
                .andExpect(status().isOk());
    }
    ...
    public void exportUserData_Success() throws Exception {
        ...
        when(privacyService.exportUserData("user123"))
                .thenReturn(userData);

        mockMvc.perform(get("/api/v1/privacy/export/user123"))
                .andExpect(status().isOk())
